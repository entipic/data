...
export class MongoRepository<T extends BaseEntity> extends BaseRepository<T> implements Repository<T> {
    constructor(protected model: MongoItem<T>, validator: IEntityValidator<T>) {
        super(validator);
    }
    ...
    innerUpdate(data: RepositoryUpdateData<T>, options?: RepositoryAccessOptions<T>): Promise<T> {
...
function optionsToMongoOptions<T extends BaseEntity>(options?: RepositoryAccessOptions<T>) {
    if (!options) {
        return undefined;
    }
